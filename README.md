...
glvpen은 KBO 공식 사이트에서 경기 데이터를 크롤링하여 **데이터 분석 인사이트를 도출**하고, 카드 기반 시뮬레이션 게임을 즐길 수 있는 플랫폼입니다. 다시즌(2020~2026) 데이터와 세이버메트릭스 지표를 활용한 분석 대시보드, AI 자동 분석 컬럼, 고급 통계 분석을 제공합니다.
...
| v3.1 | 안정화 + 시즌 대응 | AI 컬럼 시즌 fallback, 크롤러 시즌 동적화, EC2 t3.small 업그레이드 |
...
- **다시즌 크롤링**: 2020~2026 KBO 데이터 일괄 수집 (일정, 팀, 선수, 관중, 퓨처스, 게임센터)
...
AWS EC2 (t3.small, 2GB RAM) + RDS (db.t3.micro)
DuckDNS (무료 도메인) + Let's Encrypt (무료 SSL, 자동 갱신)
systemd 서비스 등록 (자동 시작/재시작)
Playwright Chromium (EC2 headless 크롤링)
```
...
| 주기 | 작업 | 비고 |
|------|------|------|
| 매일 00:00 | KBO 데이터 크롤링 (일정, 선수, 팀, 게임센터, 세이버메트릭스) | 현재 시즌 자동 감지 |
| 매주 월 06:00 | 팀별 주간 분석 AI 컬럼 자동 생성 | 시즌 데이터 없으면 전년도 fallback |
| 매주 월/목 01:00 | Python 분석 파이프라인 실행 | 4개 카테고리 로테이션 |
| 매주 목 06:00 | 15개 주제 순환 AI 특집 컬럼 자동 생성 | Python 결과 우선, 없으면 Calculator fallback |
...
| Infra | AWS EC2 (t3.small), RDS, Nginx, Let's Encrypt, systemd |
...
크롤링 대상 시즌은 동적으로 결정됩니다 (3월 이전: 전년도, 3월 이후: 현재 연도).

```bash
# 전체 크롤링 (시스템 프로퍼티로 시즌 범위 지정)
-Dcrawl.schedule=true -Dcrawl.team=true -Dcrawl.player=true
-Dcrawl.seasonStart=2020 -Dcrawl.seasonEnd=2026

# 포지션별 재크롤링
-Dcrawl.pitcherOnly=true -Dcrawl.seasonStart=2024

# 게임센터 + 세이버메트릭스
-Dcrawl.gameCenter=true -Dcrawl.sabermetrics=true
```
...
# 배포 (빌드 → EC2 전송 → 서비스 재시작)
bash deploy/deploy.sh <EC2_IP> <SSH_KEY_PATH>

# 롤백 (긴급 복구)
bash deploy/rollback.sh <EC2_IP> <SSH_KEY_PATH>

# 2026 시즌 수동 크롤링 (EC2에서 별도 프로세스로 실행)
java -Dloader.main=com.kepg.glvpen.crawler.CrawlersManualRunner \
  -Dcrawl.schedule=true -Dcrawl.team=true -Dcrawl.player=true \
  -Dcrawl.sabermetrics=true -Dcrawl.seasonStart=2026 -Dcrawl.seasonEnd=2026 \
