...
def analyze(season: int, db: DBConnector) -> tuple:
    ...
    # 투수는 ERA 기반 (낮을수록 좋음)
    val = 'ERA'
    if futures_pit.empty or val not in futures_pit.columns:
    ...
    # Top 10 유망 투수 (ERA 낮은 순)
    top10 = futures_pit.nsmallest(10, val)
    cols = [c for c in ['playerName', 'teamName', 'ERA', 'SO', 'IP', 'W', 'L', 'WPCT'] if c in top10.columns]
    stats_dict['퓨처스_투수_Top10'] = top10[cols].to_dict('records')

    # 기술통계
    for metric in ['ERA', 'SO', 'IP', 'WPCT']:
    ...
    if len(futures_pit) >= 3:
        hypothesis['퓨처스_투수_ERA_정규성'] = StatsUtils.normality_test(
            futures_pit['ERA'].dropna().values, '퓨처스 투수 ERA'
        )
        findings.append(f"퓨처스 투수 ERA 분포: {hypothesis['퓨처스_투수_ERA_정규성']['interpretation']}")
    ...
    charts.append(ChartBuilder.bar(
        '퓨처스 유망 투수 Top 10 (ERA)',
        [f"{r['playerName']}({r['teamName']})" for r in stats_dict['퓨처스_투수_Top10']],
        [{'label': 'ERA', 'data': [r['ERA'] for r in stats_dict['퓨처스_투수_Top10']]}]
    ))
    ...
    if stats_dict['퓨처스_투수_Top10']:
        ...
        radar_labels = [k for k in ['ERA', 'SO', 'IP', 'WPCT'] if k in top]
