...
public final class CrawlerUtils {
    ...
    public static Document loadPage(WebDriver driver, String url, int waitMs) throws InterruptedException {
        driver.get(url);
        int randomDelay = waitMs + ThreadLocalRandom.current().nextInt(-waitMs / 3, waitMs / 3 + 1);
        Thread.sleep(Math.max(1000, randomDelay));
    ...
    /**
     * 페이지가 로그인을 요구하는지 확인합니다.
     *
     * @param doc 확인할 Document
     * @return 로그인이 필요하면 true
     */
    public static boolean needsLogin(Document doc) {
        return doc.text().contains("로그인 후 이용 가능합니다");
    }
}
